...
export default function App() {
  ...
  const handleClearAllColumns = () => {
    ...
    const emptyCaptains = [];
    for (let i = 1; i <= 12; i++) {
      emptyCaptains.push({ id: i, teamName: `Team ${i}`, captainNick: "" });
    }
  ...
  const parseAndApplyBulkText = (textVal: string) => {
    ...
    lines.forEach((line, lineIdx) => {
      ...
      const teamName = parts[0] ? parts[0].trim() : `Team ${targetIndex + 1}`;
      const captainNick = parts[1] ? parts[1].trim() : "";

      if (targetIndex >= 0 && targetIndex < 12) {
        updatedCaptains[targetIndex] = {
          id: targetIndex + 1,
          teamName: teamName || `Team ${targetIndex + 1}`,
  ...
  const handleToggleInputMode = (targetMode: "manual" | "bulk") => {
    if (targetMode === "manual") {
      ...
      const textVal = convertCaptainsToBulkText(captains);
      setBulkText(textVal);
    }
